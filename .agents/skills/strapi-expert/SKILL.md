...
## Relation Types (Content-Type Schema)

Valid `relation` values in schema JSON: `oneToOne`, `oneToMany`, `manyToOne`, `manyToMany`, `morphToOne`, `morphToMany`, `morphOne`, `morphMany`.

**`oneWay` no existe.** Para una relación unidireccional, usa `oneToOne` (o el tipo que corresponda) sin `inversedBy`/`mappedBy` — el lado que define la relación es automáticamente el owner y se crea una joinColumn.

```json
// Unidireccional: header → navigation
"navigation": {
  "type": "relation",
  "relation": "oneToOne",
...
Para bidireccional, añade `inversedBy` o `mappedBy` en el lado correspondiente.
